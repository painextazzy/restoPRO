...
class EditTableSheet extends StatefulWidget {
  final TableModel table;
  final VoidCallback onSaved;

  const EditTableSheet({
    super.key,
    required this.table,
  ...
  @override
  State<EditTableSheet> createState() => _EditTableSheetState();
}

class _EditTableSheetState extends State<EditTableSheet> {
  ...
  void initState() {
    super.initState();
    final nomParts = widget.table.nom.split(' ');
    final numero = nomParts.length > 1 ? nomParts.last : widget.table.nom;
    _numeroController = TextEditingController(text: numero);
    _capaciteController = TextEditingController(
      text: widget.table.capacite.toString(),
    );
    _forme = widget.table.forme;
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // Vérifier si le numéro existe déjà (exclure la table en cours)
      final existing = appProvider.tables.any((table) {
        if (table.id == widget.table.id) return false;
      ...
      await appProvider.updateTable(updated);
      widget.onSaved();
      ...
      if (mounted) {
        SuccessDialog.show(
          context,
          title: 'Modifiée',
          message: 'La table a été modifiée avec succès.',
          onDismiss: () {
